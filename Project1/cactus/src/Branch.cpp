...
void Branch::setup(float _xPos,
    ...
    //===========================================
    if(ofRandom(0, 1) > splitPercentage && segmentsSinceStart != maxSegments){ //if random number between 0-1 is greater than split %, then split ... and never split on last branch(ofRandom(0, 1) > splitPercentage用于随机给一个值，判断要不要分岔)
        split = true;
    } else{
        split = false;
    }
    ...
    if(season != 2){
        for(int i = 0; i < numLeaves; i++){
            ...
            leaves.push_back(tempLeaf);//push_back画完一个新杆儿之后回到起始位置画叶子？？？
        }
    }
...
//        ofPopMatrix();
    ...
    length = length*(ofRandom(0.7, 1.2));
    angle += curvature;
    curvature += ofRandom(0, (float)(2*PI)/360.0/30);
    ...
    if (segmentsSinceStart <= maxSegments) {
        if(!split){ //if the branch does not split... just create one more branch to keep going (in roughly the same direction)
            ...
            subBranch3.setup(xPos, yPos, width, length, splitPercentage, maxSplitAngle, maxSubdivsions, maxSegments, segmentsSinceStart, angle3, curvature, color, season, 5);
        }
    }  
}
